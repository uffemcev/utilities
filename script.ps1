...
if (!(dir -Path ($env:Path -split ';') -errorAction SilentlyContinue -Force | where Name -match 'winget.exe'))
{
	start-job {
		...
		if (!(Get-AppxPackage -allusers Microsoft.DesktopAppInstaller)) {&([ScriptBlock]::Create((irm https://raw.githubusercontent.com/asheroto/winget-install/Testing/winget-install.ps1))) -Force}
