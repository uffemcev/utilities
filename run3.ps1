function install([Array]$option)
{
	...
	if ($option[0] -eq 'select') {if ((Read-Host 'enter') -eq 1) {[array]$name += 'store'}} elseif ($option -eq 'store')
	{
		Get-CimInstance -Namespace 'root\cimv2\mdm\dmmap' -ClassName 'MDM_EnterpriseModernAppManagement_AppManagement01' | Invoke-CimMethod -MethodName UpdateScanMethod
	}

	if ($option[0] -eq 'select') {if ((Read-Host 'enter') -eq 1) {[array]$name += 'office'}} elseif ($option -eq 'office')
	{
	...
	if ($option[0] -eq 'select') {if ((Read-Host 'enter') -eq 1) {[array]$name += 'dpi'}} elseif ($option -eq 'dpi')
	{
